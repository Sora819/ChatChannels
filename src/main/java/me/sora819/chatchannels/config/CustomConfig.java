...
public class CustomConfig implements ConfigAdapter{
    ...
    public CustomConfig(String sourceFileName, String targetFileName) {
        this.targetFile = new File(ChatChannelsPlugin.getInstance().getDataFolder(), targetFileName);
        this.sourceFile = sourceFileName;

        if (!targetFile.exists()) {
            targetFile.getParentFile().mkdirs();
            ChatChannelsPlugin.getInstance().saveResource(sourceFile, false);
        }
    ...
    private void addDefaults() {
        InputStream defaultFile = ChatChannelsPlugin.getInstance().getResource(sourceFile);
    ...
    public void save() {
        try {
            ...
            ChatChannelsPlugin.getInstance().getLogger().warning(LocalizationHandler.getMessage("error.save_custom_config"));
        }
    }
